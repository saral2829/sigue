var cargarcategoria = function (element, modal, modo) {
  var options = {
    type: "POST",
    url:
      "index.php?page=categorias&accion=cargarcategorias&modal=" + modal + "",
    dataType: "json",
    success: function (response) {
      ...
      $(element).fancytree({
        ...
        lazyLoad: function (event, data) {
          ...
          data.result = {
            url:
              "index.php?page=categorias&accion=cargarcategoriashijo&id=" +
              id +
              "&modal=" +
              modal +
              "",
...
$(document).on("submit", "#form_registro", function (event) {
  ...
  var detalle_unidades = $("#tabla_unidades tbody tr")
    .map(function (i, row) {
      return {
  ...
  if (
    idcategoria == "" ||
    detalle_caract.length == 0 ||
    detalle_unidades.length == 0
  ) {
    bootbox.alert(
      "<code><b>Debe completar todos los campos para el registro</b></code>"
    );
  } else {
    bootbox.confirm(
      "¿Esta seguro de realizar este registro?",
      function (result) {
...
$(document).on("click", "#add_unidad", function (event) {
  ...
  var html =
    ...
    "<td><button data-uni=" +
    s_unidades +
    ' type="button" class="btn btn-danger  remover_uni btn-xs"><i class="fa fa-fw fa-minus"></i></button></td></tr>';
