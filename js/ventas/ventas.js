    ...
    // console.log(nom_producto);
        $("#tabla_detalle > tbody").append(
            '<tr>'+
                ...
                '<td style="display :none;">'+precio_venta+'</td>'+
                '<td>'+span_precio_venta+'</td>'+
                '<td style="display :none;">'+subtotal+'</td>'+
                '<td>S/. '+subtotal+'</td>'+
                '<td style="display :none;">'+almacen+'</td>'+
...
$(document).on('click', '#btn_upt_cantidad', function(event) {
    ...
    tr.find("td").eq(5).html('<b class="prod_cant" style="cursor:pointer; color:blue"><u>'+cantidad+'</u></b>');
    tr.find("td").eq(8).html(sub_total);
    tr.find("td").eq(9).html('S7. '+sub_total);
