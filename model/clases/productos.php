...
class Productos
{
   ...
   SET nombre_producto= :nombre_producto, idcategoria= :idcategoria, idproveedor= :idproveedor, estado= :estado, user_reg = :user_reg, fh_reg = :fh_reg WHERE idproducto = :id;");
    ...
    public function generarcodebarProducto($id, $code_bar)
    {
        $stmt = $this->objPdo->prepare("UPDATE public.productos SET code_bar= :code_bar WHERE idproducto = :id;");
        $rows = $stmt->execute(array(
            'code_bar' => $code_bar,
            'id' => $id,
        ));
    ...
    {
        $stmt = $this->objPdo->prepare('SELECT * from productos where idproducto = :id;');
        $stmt->execute(array('id' => $id));
    ...
    public function registrarProducto($nombre_producto, $idcategoria, $idproveedor, $estado, $user_reg, $fh_reg, $code_bar)
    {

        $stmt = $this->objPdo->prepare("INSERT INTO public.productos(nombre_producto, idcategoria, idproveedor, estado,  user_reg, fh_reg, code_bar) VALUES (:nombre_producto, :idcategoria, :idproveedor, :estado, :user_reg, :fh_reg, :code_bar);");
        $rows = $stmt->execute(array(
            'nombre_producto' => $nombre_producto,
            'idcategoria' => $idcategoria,
            'idproveedor' => $idproveedor,
